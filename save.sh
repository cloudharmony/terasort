...
Saves TeraSort test results to CSV files, Google BigQuery, MySQL or PostgreSQL. 
...
--store_prefix              If the --store argument is set, this argument 
                            ...
                            values may be included:
                              ...
                              {benchmark}     => benchmark name (geekbench)
                                                 (meta-id value in benchmark.ini)
                              {version}       => benchmark version (e.g. 3_1_6)
